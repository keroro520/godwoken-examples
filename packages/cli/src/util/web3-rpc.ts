...
import {
    WithdrawalRequest,
    WithdrawalRequestCodec,
    WithdrawalRequestExtra,
    WithdrawalRequestExtraCodec
} from "../schema/schema";
...
export class GodwokenWeb3Rpc {
    ...
    public async getNonce(ethAddress: EthAddress): Promise<number> {
        const hexNonce = (await this.call_("eth_getTransactionCount", ethAddress, "latest")) as HexNumber;
    ...
    public async getWithdrawal(withdrawalHash: Hash) : Promise<{status: string}>{
        return (await this.call_("gw_get_withdrawal", withdrawalHash, 1)) as {status: string};
    }

    public async submitWithdrawalRequest(withdrawalRequestExtra: WithdrawalRequestExtra) : Promise<Hash> {
        const serialized= (new WithdrawalRequestExtraCodec(withdrawalRequestExtra)).HexSerialize();
        return (await this.call_("gw_submit_withdrawal_request", serialized)) as Hash;
    }
